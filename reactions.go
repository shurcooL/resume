...
func postReaction(emojiID string, reactableID string) error {
	reactableURL := dom.GetWindow().Location().Href + reactableID // TODO Get proper location without hash.
	resp, err := http.PostForm("/TODO/react", url.Values{"reactableURL": {reactableURL}, "reaction": {emojiID}})
	if err != nil {
		return err
	}
